...
- `terrarium_spawn` — run one bounded child agent task. Existing callers remain synchronous when `background` is omitted. With `background: true`, **do not sleep or poll in Pi**: the Terrarium Pi extension automatically subscribes the current session to that concrete run, and terminal completion is surfaced as described below. On hosts without the extension, use the low-level callback pull API or `terrarium_status`. Retries remain synchronous.
...
When installed as a Pi package, Terrarium adds a thin native widget for active runs/groups, claimed-and-acknowledged completion messages, and `/terrarium-status`, `/terrarium-groups`, and `/terrarium-cancel`. A background `terrarium_spawn` is automatically attached to the **current Pi session by concrete run ID**. The Pi subscriber is not a wildcard channel listener, so sibling Pi sessions in the same cwd/channel are not woken for runs they did not spawn. On terminal completion, the extension atomically claims the durable callback, sends a visible follow-up with `triggerTurn: true`, then acknowledges it. Pi therefore starts a model turn immediately when idle, or queues the callback until the active turn finishes. If Pi is closed, the subscriber/mailbox survives shutdown and the callback is replayed and surfaced when that session resumes. Finish-before-subscribe is covered by the concrete-run journal replay. Spawned children do not load this observer layer. This is host delivery, not conversation memory or session continuation; hosts without the extension remain pull-based.
