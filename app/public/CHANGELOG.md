...
- **New `terrarium_report_failure` MCP tool + `src/failure-report.js` core.** Terrarium already catches failures — a child whose process exits without a trusted, contract-matching receipt is marked failed, never a win. This turns one such caught failure into a **structured, deduped bug report**: fetch the run's terminal status + log (cloud or local, by runId alone), classify it (`receipt-mismatch`/`receipt-absent`/`receipt-malformed`/`agent-timeout`/`model-config`/`ca-trust`/`poll-timeout`/…), assign a blame hint (`agent`/`backend`/`image`), redact + head/tail-excerpt the log, and file it under `~/.terrarium/failure-reports`. Pass `markdown:true` for a ready-to-paste bug-report body.
- **Defect-level dedupe.** The dedupe signature keys on the failure class + sub-reason + exit code + blame, *not* the runId or timestamp — so N runs failing the same way collapse into ONE report with an `occurrences` count and a `seenRunIds` list, instead of N near-identical files. A trusted success or still-running run is refused (`not-a-failure`).
- **Honesty contract, made durable.** This is the “a child claim is not proof” rule turned into a persisted artifact: a caught failure is no longer just observed in a status poll — it becomes a filed, triageable record. Scoped to top-level controllers (like `terrarium_doctor`); nested children cannot file global reports. Covered by `test/failure-report.test.js` (10 tests: classification of the real `mismatch:runId`/timeout/malformed cases, trusted-success refusal, log redaction, dedupe-collapse vs distinct-defect separation, markdown render).
- Surfaces (not fixes) an underlying weakness it caught: the fixed cloud child model does not reliably echo the assigned `runId` verbatim into its receipt line, so real work fails the contract as `mismatch:runId`. The report names it and blames `agent`; the model/runner fix is tracked separately.

## 2026-07-20 — Web consoles sign in with GitHub (no token-paste)

- **The `/runs` and `/batches` consoles now authenticate with a GitHub sign-in**, not a pasted control token. `/auth/login` runs the GitHub OAuth web flow and sets an HttpOnly, HMAC-signed session cookie the browser sends automatically; the page never holds a token. Only the one configured GitHub login (`GITHUB_ALLOWED_LOGIN`, the instance owner) may sign in. Sessions last 12h. The programmatic Bearer path (`Authorization: Bearer …`) is unchanged for the CLI, MCP, and other services — the API now accepts *either* a valid bearer *or* a valid session cookie, both resolving to the same owner principal.
- Fail-closed: with no OAuth config (`GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET`/`SESSION_SECRET`/`GITHUB_ALLOWED_LOGIN`), `/auth/*` refuses and the consoles show "sign in" but cannot complete a login — the API still requires a bearer, so nothing opens up by default. Covered by `test/cloud-web-session.test.js` (16 tests: signed-cookie round-trip + tamper/expiry rejection, wrong-login rejection, bearer-or-cookie owner auth, OAuth state check, no open redirect, HttpOnly/Secure/SameSite cookie).
