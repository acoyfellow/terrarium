...
function renderDocs() {
  ...
  const pages = {
    ...
    explain: shell('Explanation: operational truth', 'Explanation', `<h2>Proof chain</h2><p>The authoritative success proof is child exit 0 plus a verified <code>TERRARIUM_RESULT</code> receipt with matching run id, task fingerprint, and nonce. Exit 0 alone is process state.</p><h2>Callbacks are wakeups</h2><p>Callbacks and Pulse are at-least-once terminal notifications with dedup. They tell a consumer to look; they do not prove task success.</p><h2>Groups are rollups</h2><p>Groups fail closed and report <code>contractTruth</code>. They never replace per-run receipts.</p><h2>Isolation is not a sandbox</h2><p><code>copy</code> and <code>worktree</code> separate workspaces. They do not create a security sandbox.</p><h2>Engine</h2><p>TypeScript is the product engine. Go code remains internal conformance/research material, not an operator-facing alternate engine.</p>`),
